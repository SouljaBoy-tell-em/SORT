...
int main (void) {
    ...
    statusMemory (mem_start); //!TODO: ??????????? ? define
    // --------------
    ...
    copyBuf (mem_start, copy_mem_start); // !TODO: ??????????? ????????? ? ????????? ???????
    // --------------
...
void copyBuf (const char * mem_start, char * buffer) { // !TODO: ????????????? ???????
...
unsigned long FileSize (FILE * file, struct stat * buf) { // const char * st;
    ...
    fstat (fileno (file), buf); // !TODO: ?????????? fstat
